...
public class Consumer {
    ...
    public void receive(ConsumerRecord<?, ?> consumer, Acknowledgment ack) throws SQLException {
        GoldenGateMsg ggm = JsonUtils.fromJson(consumer.value().toString(), GoldenGateMsg.class);
        //提取变化前的数据
        Map<String, String> before = ggm.getBefore() == null ? null : JsonUtils.fromJson(ggm.getBefore().toString(), Map.class);
        //提取变化后的数据
        Map<String, String> after = ggm.getAfter() == null ? null : JsonUtils.fromJson(ggm.getAfter().toString(), Map.class);
        //获取缓存名
